...
napi_value sendPacket1(napi_env env, napi_callback_info info) {
	size_t argc = 3;
	napi_value args[3];
	napi_get_cb_info(env, info, &argc, args, NULL, NULL);

	int peer_num;
	napi_get_value_int32(env, args[0], &peer_num);
	...
	bool ret = sendPacket(peer_num, data, length, channel);
...
napi_value netLoop1(napi_env env, napi_callback_info info) {
	...
	if(event.peer){
		int peer_num = peer_num_by_peer(event.peer);
		napi_create_int32(env, (int)peer_num, &value);
...
napi_value initialize_client1(napi_env env, napi_callback_info info) {
	size_t argc = 3;
	napi_value args[3];
	napi_get_cb_info(env, info, &argc, args, NULL, NULL);
...
napi_value Init(napi_env env, napi_value exports) {
	...
	napi_create_function(env, NULL, 0, initialize_client1, NULL, &fn);
	napi_set_named_property(env, exports, "initialize_client", fn);

	napi_create_function(env, NULL, 0, netLoop1, NULL, &fn);
	napi_set_named_property(env, exports, "netLoop", fn);

	napi_create_function(env, NULL, 0, sendPacket1, NULL, &fn);
